# Zenith per-component Docker images
...
| Language Server | `Dockerfile.lsp`       | ✅ Working | `zenith-lsp` — real stdio LSP server. Diagnostics come from the actual lexer/parser, not mocked. Run with `docker run -i`. |
| Build System / Package Manager | `Dockerfile.buildsystem` | ✅ Working | `zenith-pkg` — real `Zenith.toml` TOML parsing + dependency resolution. Registry fetch/publish and multi-target codegen are simulated (clearly logged as such) — no real registry exists yet. |
...
3. Split `full` into independently-buildable features (`lsp`, `buildsystem`)
   ...
   to compile first.
4. Added real entrypoints: `zenith-lsp` (stdio LSP backed by the actual
   parser) and `zenith-pkg` (manifest parsing + dependency resolution),
...
```bash
# Compiler
docker build -f docker/Dockerfile.compiler -t zenith/compiler .
docker run --rm zenith/compiler zenith --help

# Language server (speaks LSP over stdio — wire into an editor, e.g. Neovim/VSCode)
docker build -f docker/Dockerfile.lsp -t zenith/lsp .
docker run -i --rm zenith/lsp

# Build system / package manager
docker build -f docker/Dockerfile.buildsystem -t zenith/pkg .
docker run --rm -v "$(pwd)":/project zenith/pkg deps --manifest Zenith.toml
```
