...
fn parse_ok(src: &str) -> Program {
    ...
    assert!(p.get_errors().is_empty(),
        "Parse errors for {:?}: {:?}", src, p.get_errors());
    prog
}
...
#[test] fn test_fn_with_return_type() {
...
#[test] fn test_fn_no_params_no_ret() {
    let prog = parse_ok("fn hello() { let x = 1; }");
    assert!(matches!(&prog.statements[0], Statement::Function(_, _, _, None, _)));
}

#[test] fn test_fn_with_default_param() {
...
#[test] fn test_if_only() {
    let prog = parse_ok("if x { let a = 1; }");
    assert!(matches!(&prog.statements[0], Statement::Expression(Expression::If(_, _, _, None))));
}

#[test] fn test_if_else_if_else() {
    let prog = parse_ok("if a { let x = 1; } else if b { let y = 2; } else { let z = 3; }");
    assert_eq!(prog.statements.len(), 1);
}

#[test] fn test_while_complex() {
...
#[test] fn test_match_multiple_arms() {
...
#[test] fn test_quantum_circuit_named() {
...
#[test] fn test_quantum_circuit_with_params() {
    let prog = parse_ok("quantum circuit GHZ(n) { let q = n; }");
    assert!(matches!(&prog.statements[0], Statement::QuantumCircuit(_, _, _)));
}

#[test] fn test_quantum_op_expr() {
...
#[test] fn test_remember_with_type() {
    let prog = parse_ok("remember past: History = 999;");
    assert!(matches!(&prog.statements[0], Statement::SankofaMemory(_, _, _)));
}

#[test] fn test_recall_expr() {
    let prog = parse_ok("let mem = recall(past);");
    assert_eq!(prog.statements.len(), 1);
}

#[test] fn test_learn_expr() {
...
#[test] fn test_type_alias() {
    ...
    assert!(matches!(&prog.statements[0], Statement::TypeDeclaration(_, _, _)));
}

#[test] fn test_struct_decl() {
    let prog = parse_ok("struct Point { x, y, }");
    assert_eq!(prog.statements.len(), 1);
}

#[test] fn test_struct_with_types() {
    let prog = parse_ok("struct Patient { id: PatientId, age: Int, }");
    assert_eq!(prog.statements.len(), 1);
}

#[test] fn test_enum_decl() {
...
#[test] fn test_interface_decl() {
    let prog = parse_ok("interface Runnable { fn run(); }");
    assert!(matches!(&prog.statements[0], Statement::Interface(_, _, _, _)));
}

// ── Module / import ───────────────────────────────────────────────────────────

#[test] fn test_import_stmt() {
    let prog = parse_ok("import stdlib.math;");
    assert!(matches!(&prog.statements[0], Statement::Import(_, _)));
}

#[test] fn test_module_decl() {
...
#[test] fn test_nested_function_calls() {
    let prog = parse_ok("let r = outer(inner1(a), inner2(b, c));");
    assert_eq!(prog.statements.len(), 1);
}

#[test] fn test_index_access() {
...
#[test] fn test_assign_expr() {
    let prog = parse_ok("x = 10;");
    assert_eq!(prog.statements.len(), 1);
}

#[test] fn test_complex_arithmetic() {
...
#[test] fn test_zamani_block() {
    let prog = parse_ok("let past = zamani { let x = 1; };");
    assert_eq!(prog.statements.len(), 1);
}

#[test] fn test_sasa_block() {
...
#[test] fn test_ir_from_function() {
    let module = parse_and_ir("fn add(a, b) { return a; }");
    assert!(module.functions.iter().any(|f| f.name == "add"),
        "Expected 'add' in IR functions");
}

#[test] fn test_ir_from_let_bindings() {
    let module = parse_and_ir("let x = 1; let y = 2; let z = x;");
    let top = module.functions.iter().find(|f| f.name == "__top__").unwrap();
    assert!(!top.body.is_empty());
}

#[test] fn test_ir_from_quantum_circuit() {
    let module = parse_and_ir("quantum circuit Bell { let q = 1; }");
    let top = module.functions.iter().find(|f| f.name == "__top__").unwrap();
    assert!(!top.body.is_empty());
}

#[test] fn test_ir_from_sankofa() {
    let module = parse_and_ir("remember wisdom = 42;");
    let top = module.functions.iter().find(|f| f.name == "__top__").unwrap();
    let has_sankofa = top.body.iter().any(|ins| matches!(ins, zenith_compiler::ir_gen::IrInstruction::SankofaStore(_, _)));
    assert!(has_sankofa, "Expected SankofaStore in IR");
}

#[test] fn test_ir_while_loop() {
    let module = parse_and_ir("while true { let x = 1; }");
    let top = module.functions.iter().find(|f| f.name == "__top__").unwrap();
    let has_cond_jump = top.body.iter().any(|ins| matches!(ins, zenith_compiler::ir_gen::IrInstruction::CondJump(_, _, _)));
    assert!(has_cond_jump, "Expected CondJump in IR for while loop");
}

// ── Optimizer ─────────────────────────────────────────────────────────────────

#[test] fn test_optimizer_constant_folding() {
    let mut module = parse_and_ir("let r = 2 + 3;");
    let mut opt = Optimizer::new();
    opt.run_all(&mut module);
    assert!(!module.functions.is_empty());
}

#[test] fn test_optimizer_dead_code_elimination() {
    let mut module = parse_and_ir("let x = 1; let y = 2; let z = 3;");
    let mut opt = Optimizer::new();
    opt.run_all(&mut module);
    assert!(!module.functions.is_empty());
}
...
#[test] fn test_full_zenith_program() {
    let src = r#"
        effect QuantumError;
    ...
    "#;
    let prog = parse_ok(src);
    assert!(prog.statements.len() >= 5, "Expected at least 5 top-level declarations");
}

#[test] fn test_class_with_methods() {
    let src = r#"
        class Calculator {
            value: Int,
            fn add(x: Int) -> Int {
...
#[test] fn test_module_with_multiple_fns() {
    let src = r#"
        module math_utils {
