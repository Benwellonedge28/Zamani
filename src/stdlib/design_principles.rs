...
pub struct DesignAuditor {
    violations: Vec<DesignViolation>,
}
impl DesignAuditor {
    pub fn new() -> Self {
        DesignAuditor {
    ...
    pub fn audit(&mut self, code: &str) -> Vec<DesignViolation> {
        ...
        if code.contains("global") {
            v.push(DesignViolation {
                principle: DesignPrinciple::Solid,
                description: "Possible global state".into(),
                severity: 0.6,
            });
        }
        if code.contains("copy") {
            v.push(DesignViolation {
                ...
                description: "Potential duplication".into(),
                severity: 0.4,
            });
        }
        self.violations.extend(v.clone());
        v
    }
    pub fn score(&self) -> f32 {
        1.0 - self
            ...
            .sum::<f32>()
            .min(1.0)
            / self.violations.len().max(1) as f32
    }
}
impl Default for DesignAuditor {
    fn default() -> Self {
        Self::new()
    }
}
