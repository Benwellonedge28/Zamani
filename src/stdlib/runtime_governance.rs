...
impl RuntimeGovernor {
    pub fn new() -> Self {
        RuntimeGovernor {
            policies: Vec::new(),
            log: VecDeque::with_capacity(10000),
        }
    }
    pub fn add_policy(&mut self, id: &str, rule: &str) {
    ...
    pub fn evaluate(&mut self, action: &str, actor: &str, ts: u64) -> PolicyOutcome {
        let outcome = if self
            .policies
            .iter()
            .any(|p| p.enforced && action.contains(&p.rule))
        {
    ...
    pub fn compliance_rate(&self) -> f32 {
        ...
        if t == 0 {
            ...
            1.0 - self
                .log
                .iter()
                .filter(|e| e.outcome == PolicyOutcome::Denied)
                .count() as f32
                / t as f32
        }
    }
