...
impl RagEngine {
    ...
    pub fn retrieve(&self, query: &str, top_k: usize) -> Vec<&Document> {
        ...
        let mut scored: Vec<(usize, &Document)> = self
            .docs
            .iter()
            .map(|d| (qw.iter().filter(|&&w| d.content.contains(w)).count(), d))
            .collect();
        ...
        RagResult {
            answer: if grounded {
                format!(
                    "Based on {} docs: {}",
                    docs.len(),
                    &docs[0].content[..docs[0].content.len().min(80)]
                )
            } else {
                format!("No docs for: {}", query)
            },
            docs,
            hallucination_score: if grounded { 0.05 } else { 0.8 },
            grounded,
        }
    }
    pub fn detect_hallucination(&self, text: &str, sources: &[Document]) -> HallucinationReport {
        let claims: Vec<&str> = text.split(". ").collect();
        let grounded: Vec<String> = claims
            .iter()
            .filter(|&&c| sources.iter().any(|s| s.content.contains(c)))
            .map(|&c| c.into())
            .collect();
        let unsupported: Vec<String> = claims
            ...
            .collect();
        HallucinationReport {
            risk: unsupported.len() as f32 / claims.len().max(1) as f32,
            unsupported,
            grounded,
        }
    }
