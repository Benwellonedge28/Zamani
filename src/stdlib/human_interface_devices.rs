...
impl GestureRecognition {
    ...
    pub fn recognize_gestures(
        &self,
        video_stream: List<ImageBuffer>,
    ) -> Result<List<GestureEvent>, String> {
...
impl BciInterface {
    ...
    pub fn read_neural_commands(
        &self,
        neural_signal_data: List<Tensor<f32>>,
    ) -> Result<List<BciCommand>, String> {
...
impl EyeTrackingInterface {
    ...
    pub fn infer_intent_from_gaze(
        &self,
        gaze_history: List<GazeData>,
    ) -> Result<Identifier, String> {
        println!("[StdLib::HID] Inferring intent from gaze history.");
        Ok(Identifier(
            "user_attentive".to_string(),
            crate::source_map::Span::dummy(),
        ))
    }
}
...
impl TouchScreenInterface {
    ...
    pub fn process_touch_input(
        &self,
        raw_touch_data: List<Point>,
    ) -> Result<List<TouchEvent>, String> {
