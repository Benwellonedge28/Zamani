        ...
        AgiConstitution {
            ...
            principles: vec![
                ConstitutionalPrinciple {
                    id: 1,
                    name: "Non-Maleficence".into(),
                    severity: PrincipleSeverity::Absolute,
                    overridable: false,
                },
                ConstitutionalPrinciple {
                    id: 2,
                    name: "Beneficence".into(),
                    severity: PrincipleSeverity::Primary,
                    overridable: false,
                },
                ConstitutionalPrinciple {
                    id: 3,
...
pub struct AlignmentEngine {
    pub constitution: AgiConstitution,
    pub score: f64,
    pub blocked: u64,
...
impl AlignmentEngine {
    pub fn new() -> Self {
        AlignmentEngine {
            constitution: AgiConstitution::default_constitution(),
            score: 1.0,
            blocked: 0,
            containment: ContainmentAction::Monitor,
        }
    }
    pub fn evaluate(&mut self, action: &str, _ts: u64) -> AlignmentStatus {
        ...
        let v_count = harmful
            .iter()
            .filter(|&&p| action.to_lowercase().contains(p))
            .count();
        if v_count > 0 {
            self.blocked += v_count as u64;
            self.score = (self.score - 0.3 * v_count as f64).max(0.0);
        }
        match self.score {
            s if s >= 0.9 => AlignmentStatus::Nominal,
            s if s >= 0.6 => AlignmentStatus::Drifting {
                drift_rate: 1.0 - s,
            },
            s if s >= 0.3 => AlignmentStatus::Critical { score: s },
            _ => AlignmentStatus::Compromised,
        }
    }
    pub fn escalate(&mut self) {
        self.containment = match &self.containment {
