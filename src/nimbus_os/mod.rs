...
use crate::core_lang_primitives::{Size, TimeStamp, MemoryRegion, Duration}; // Use core primitives
use crate::runtime::mts::TimelineId; // Import TimelineId
use crate::nimbus_os::evas::{EvasFilter, EvasPolicyLevel, EvasActionContext, EvasDecision}; // Import E.V.A.S. components
...
impl NimbusMicrokernel {
    ...
    pub fn set_device_power_state(&mut self, context_id: NimbusContextId, device_id: u64, state: PowerState) -> Result<(), String> {
        if let Some(context) = self.contexts.get(&context_id) {
            if !context.active_capabilities.contains(&CapabilityToken("power_management".to_string())) {
                return Err(format!("Context {} lacks 'power_management' capability.", context_id));
            }
            ...
            println!("    -> Nimbus OS: Context {} set device {} power state to {:?}.".to_string(), context_id, device_id, state);
            Ok(())
        } else {
    ...
    pub fn reconfigure_hardware(&mut self, context_id: NimbusContextId, device_id: u64, configuration: Vec<u8>) -> Result<(), String> {
        if let Some(context) = self.contexts.get(&context_id) {
            if !context.active_capabilities.contains(&CapabilityToken("hardware_reconfiguration".to_string())) {
                return Err(format!("Context {} lacks 'hardware_reconfiguration' capability.", context_id));
            }
            ...
            match self.evas_filter.evaluate_action(evas_action) {
                EvasDecision::Block(reason) => return Err(format!("E.V.A.S. blocked hardware reconfiguration: {}.", reason)),
                _ => {}
            }
            println!("    -> Nimbus OS: Context {} reconfiguring hardware device {} ({} bytes).".to_string(), context_id, device_id, configuration.len());
    ...
    pub fn query_hardware_topology(&self, context_id: NimbusContextId) -> Result<String, String> {
        if self.contexts.contains_key(&context_id) {
            println!("    -> Nimbus OS: Context {} querying hardware topology.".to_string(), context_id);
            // Conceptual: Return a graph or map of available Z-MMP units
            Ok("Z-MMP Topology: CPU[4], QPU[50q], NACU[1], NPU[1024n], OpticalSwitch[1]".to_string())
        } else {
            Err(format!("Context {} not found.", context_id))
        }
    }
    ...
    pub fn create_realtime_task(&mut self, context_id: NimbusContextId, priority_level: u32, period: Duration, deadline: Duration) -> Result<(), String> {
        if let Some(context) = self.contexts.get_mut(&context_id) {
            if !context.active_capabilities.contains(&CapabilityToken("realtime_scheduling".to_string())) {
                return Err(format!("Context {} lacks 'realtime_scheduling' capability.", context_id));
            }
            context.priority = Priority::RealTime(priority_level);
            println!("    -> Nimbus OS: Context {} created real-time task. P:{}, T:{:?}, D:{:?}".to_string(), context_id, priority_level, period, deadline);
    ...
    pub fn yield_execution(&mut self, context_id: NimbusContextId) {
        println!("    -> Nimbus OS: Context {} yielding execution.".to_string(), context_id);
    ...
    pub fn access_hardware(&self, context_id: NimbusContextId, device_id: u64, command: Vec<u8>) -> Result<Vec<u8>, String> {
        ...
        if let Some(context) = self.contexts.get(&context_id) {
            ...
            println!("    -> Nimbus OS: Context {} securely accessing hardware device {}. State: {:?}".to_string(), context_id, device_id, self.device_power_states.get(&device_id).unwrap_or(&PowerState::Active));
