...
#[derive(Debug, Clone, PartialEq)]
pub enum BondType { Single, Double, Triple, Aromatic, Hydrogen, VanDerWaals }
...
#[derive(Debug, Clone, PartialEq)]
pub enum NanoAgentState { Idle, Moving, Assembling, Disassembling, Communicating, Replicating, Error(String) }
...
#[derive(Debug, Clone, PartialEq)]
pub enum ArchaeveEncoding { DnaBase4, ProteinFolding, CrystalLattice, QuantumDot }

impl<T: Clone> Archaeve<T> {
    pub fn new(encoding: ArchaeveEncoding, durability_years: f32) -> Self {
        Archaeve { data: Vec::new(), encoding, durability_years }
    }
    pub fn store(&mut self, item: T) { self.data.push(item); }
    pub fn retrieve(&self, index: usize) -> Option<&T> { self.data.get(index) }
...
impl NanoRuntime {
    pub fn new() -> Self { NanoRuntime { agents: HashMap::new(), next_id: 1, tick: 0 } }

    pub fn spawn_agent(&mut self, blueprint: &str, components: Vec<String>) -> u64 {
        let id = self.next_id; self.next_id += 1;
        self.agents.insert(id, NanoAgent { id, blueprint: blueprint.into(), components, state: NanoAgentState::Idle, position: [0.0, 0.0, 0.0], energy_uj: 1.0 });
        id
    }
    ...
    pub fn tick(&mut self) {
        ...
        for a in self.agents.values_mut() {
            if a.energy_uj > 0.0 { a.energy_uj -= 0.001; } else { a.state = NanoAgentState::Error("No energy".into()); }
        }
    }
...
impl Default for NanoRuntime { fn default() -> Self { Self::new() } }
