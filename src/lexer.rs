...
impl Lexer {
    ...
    fn read_number(&mut self, first_digit: char) -> String {
        ...
        while let Some(&c) = self.peek_char() {
            if c.is_digit(10) || (c == '.' && self.peek_char_n(2).map_or(false, |next_c| next_c.is_digit(10))) {
        ...
        if let Some(c) = current_char {
            ...
            if c == '|' {
                if self.peek_char() == Some(&' ') { // This might be start of a quantum literal
                    ...
                    return Some(Token::new(TokenType::Pipe, "|", start_pos));
                }
            }
            ...
            match c {
                '=' => {
                    if self.peek_char() == Some(&'=') {
                        self.read_char();
                        Some(Token::new(TokenType::Equals, "==", start_pos))
                    } else {
                        Some(Token::new(TokenType::Assign, "=", start_pos))
                    }
                }
                '!' => {
                    if self.peek_char() == Some(&'=') {
                        self.read_char();
                        Some(Token::new(TokenType::NotEquals, "!=", start_pos))
                    } else {
                        Some(Token::new(TokenType::Bang, "!", start_pos))
                    }
                }
                '<' => {
                    if self.peek_char() == Some(&'=') {
                        self.read_char();
                        Some(Token::new(TokenType::LTE, "<=", start_pos))
                    } else {
                        Some(Token::new(TokenType::LT, "<", start_pos))
                    }
                }
                '>' => {
                    if self.peek_char() == Some(&'=') {
                        self.read_char();
                        Some(Token::new(TokenType::GTE, ">=", start_pos))
                    } else {
                        Some(Token::new(TokenType::GT, ">", start_pos))
                    }
                }
                ...
                '"' => {
                    ...
                    self.read_char(); // This moves past the closing quote
...
pub mod tokens {
    ...
    pub enum TokenType {
        ...
        Semicolon, Colon, Comma, Dot, Pipe, // Added Pipe
