...
use std::sync::Arc; // For keywords_map
...
impl Lexer {
    ...
    fn read_char(&mut self) {
        ...
        if self.read_position.0 >= self.input.len() as u32 {
            ...
            if let Some((byte_offset, ch)) = self
                .input
                .char_indices()
    ...
    fn read_identifier(&mut self) -> String {
        ...
        self.input
            .get(position as usize..self.position.0 as usize)
            .unwrap_or("")
            .to_string()
    }
    ...
    fn read_string(&mut self) -> String {
        ...
        while let Some(c) = self.ch {
            ...
            if c == '\\' {
                // Handle escape sequences conceptually
    ...
    fn read_char_literal(&mut self) -> String {
        ...
        if self.ch == Some('\\') {
            // Handle backslash escape explicitly
            self.read_char(); // Consume backslash
            self.read_char(); // Consume escaped char
    ...
    pub fn next_token(&mut self) -> Token {
        ...
        match self.ch {
            ...
            Some('|') => {
                if self.peek_char() == Some('|') {
                    ...
                    literal = self
                        .input
                        .get(start_pos.0 as usize..self.position.0 as usize)
                        .unwrap_or("")
                        .to_string();
        ...
        if literal.is_empty() {
            literal = self
                .input
                .get(start_pos.0 as usize..end_pos.0 as usize)
                .unwrap_or("")
                .to_string();
        }
...
mod lexer_unit_tests {
    ...
    fn test_let_keyword() {
        ...
        println!(
            "keywords_map has let: {}",
            lexer.keywords_map.contains_key("let")
        );
        assert_eq!(
            tok1.token_type,
            TokenType::KeywordLet,
            "let should be KeywordLet"
        );
    }
}
