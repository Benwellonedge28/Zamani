...
impl PackageManager {
    ...
    pub fn resolve_dependencies(
        ...
        let mut to_resolve_queue: Vec<(String, String)> = manifest
            .dependencies
            .iter()
            .filter_map(|(name, dep_cfg)| {
                // Filter out local path deps for remote resolution
                if dep_cfg.path.is_none() {
                    Some((name.clone(), dep_cfg.version.clone()))
                } else {
                    None
                }
            })
        ...
        println!(
            "[Toolchain::Pkg] Resolved {} direct and transitive dependencies.",
            resolved_deps.len()
        );
        Ok(resolved_deps)
    }
