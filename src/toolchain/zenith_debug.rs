...
use crate::runtime::debugger::{ZenithDebugger, Breakpoint, DebuggerState, StackFrame, Variable};
...
impl ZenithDebugAdapter {
    ...
    pub fn on_scopes(&mut self, frame_id: u64) -> Result<List<Scope>, String> {
        println!("[zenith-debug] Retrieving scopes for frame {}.".to_string(), frame_id);
    ...
    pub fn on_variables(&mut self, variables_reference: u64) -> Result<List<Variable>, String> {
        println!("[zenith-debug] Retrieving variables for reference {}.".to_string(), variables_reference);
    ...
    pub fn send_debug_event(&mut self, event: DebugEvent) {
        println!("[zenith-debug] Sending debug event: {}.".to_string(), event.to_string());
...
impl RuntimeInterface {
    ...
    pub fn launch_or_attach(&mut self, config: Map<String, MetaValue>) -> Result<(), String> { Ok(()) }
}
...
#[derive(Debug, Clone, PartialEq)]
pub enum DebugEvent { Initialized, Stopped, Continued, Exited } // Dummy
impl ToString for DebugEvent { fn to_string(&self) -> String { format!("{:?}", self) } }
