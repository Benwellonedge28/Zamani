...
# Zamani Package Registry Specification

This document outlines the conceptual design and API for the official Zamani Package Registry. The registry serves as a central repository for discovering, sharing, and distributing `.zpkg` (Zamani Package) files. It is an integral part of the Zamani ecosystem, working in conjunction with the `zamani-pkg` package manager.
...
*   **Unique Name:** Each package has a globally unique name (e.g., `zamani_linalg`, `quantum_simulator_lib`, `nano_swarm_utils`).
...
*   **Qualified Names:** When needed, packages can be referred to by a fully qualified name, e.g., `registry.zamani-lang.org/my_org/my_package@1.0.0`.

## 3. Registry API (Conceptual RESTful API)

The Zamani Package Registry exposes a RESTful API for interaction by `zamani-pkg` and other tools.

### 3.1. Authentication

*   All write operations (publish, yank, delete) require authentication (e.g., OAuth2, API tokens linked to Zamani developer accounts).
...
*   **Response:** `200 OK` with a `PackageVersionDetails` object. This includes the parsed `Zamani.toml`, dependency tree, verified properties, and download URL.
...
*   **Process:**
    1.  The registry extracts the `Zamani.toml` from the `.zpkg`.
    2.  Validates the `Zamani.toml` structure and version.
    3.  Performs security scans (`E.V.A.S.`) on the package content.
    4.  Submits the package to a conceptual **Formal Verification Service** (leveraging `zamani-fv`).
...
*   **Description:** Marks a package version as "yanked", meaning it will no longer be resolved by `zamani-pkg` for new projects, but existing projects can still access it.
...
Beyond the basic `Zamani.toml` contents, the registry stores additional metadata:
...
1.  Extracts Zamani source from `.zpkg`.
2.  Runs `zamanic` with `zamani-fv` integration, applying a predefined set of verification properties (e.g., `MemorySafety`, `CausalConsistency`, `EntanglementPurity`).
...
## 6. `zamani-pkg` Interaction Flow (Conceptual)

1.  **`zamani-pkg install <package>`:**
...
This conceptual specification provides a detailed vision for a robust, secure, and multi-paradigm-aware package registry for Zamani.
